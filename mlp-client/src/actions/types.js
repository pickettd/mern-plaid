export const GET_ERRORS = "GET_ERRORS";
export const USER_LOADING = "USER_LOADING";
export const SET_CURRENT_USER = "SET_CURRENT_USER";
export const ADD_ACCOUNT = "ADD_ACCOUNT";
export const REFRESH_ACCOUNT = "REFRESH_ACCOUNT";
export const UPDATE_ACCOUNT = "UPDATE_ACCOUNT";
export const DELETE_ACCOUNT = "DELETE_ACCOUNT";
export const GET_ACCOUNTS = "GET_ACCOUNTS";
export const ACCOUNTS_LOADING = "ACCOUNTS_LOADING";
export const GET_TRANSACTIONS = "GET_TRANSACTIONS";
export const TRANSACTIONS_LOADING = "TRANSACTIONS_LOADING";
export const SET_BUDGETS = "SET_BUDGETS";
export const SET_CATEGORY_MAP = "SET_CATEGORY_MAP";
export const SET_INCOME_SUM = "SET_INCOME_SUM";
export const SET_SPENDING_SUM = "SET_SPENDING_SUM";
export const SET_SPEND_RANGE_DAYS_SELECTED = "SET_SPEND_RANGE_DAYS_SELECTED";
export const SET_TOTAL_TRANSACTION_COUNT = "SET_TOTAL_TRANSACTION_COUNT";
export const SET_REVIEWED_TRANSACTION_COUNT = "SET_REVIEWED_TRANSACTION_COUNT";
export const SET_CATEGORIES_THIS_SPEND_RANGE =
  "SET_CATEGORIES_THIS_SPEND_RANGE";
export const SET_SPENDING_BY_CATEGORY = "SET_SPENDING_BY_CATEGORY";
